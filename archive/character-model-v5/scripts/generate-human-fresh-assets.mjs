...
const modelId = 'adventurer-fresh-v5';
const modelVersion = 5;

// v5 keeps the runtime-compatible 96px frame and the established 48-unit
// design space, but replaces the model rig and cosmetic construction. The
// artwork deliberately uses chunky two-pixel clusters, sparse facial marks and
// one shared eight-direction anchor contract inspired by the original game's
// most readable mage sprites.
const designSize = 48;
const detailScale = 2;
...
function drawHeritage(image, p, style, raceId = 'human') {
  ...
  if (raceId === 'human') {
    if (style === 'road-freckles') {
      ...
      for (const [dx, dy] of [[-1.1, 1.8], [0.15, 2.1], [1.2, 1.75], [2.05, 2.55]]) {
  ...
  if (raceId === 'elf') {
    if (style === 'moon-sigil') {
      ...
      line(image, x - (profile ? side * 3 : 5), y - 4, x + (profile ? side * 3 : 5), y - 4, C.goldDeep, 0.6);
      detail(image, x + (profile ? side : 0), y - 4, C.goldLight);
      detail(image, x + (profile ? side : 0), y - 4.75, '#c4b5fd');
    }
    return;
  }
...
// The v5 rig is intentionally separate from the archived v4 construction.
...
function poseV5(row, column, raceId = 'human') {
  const direction = directions[row];
  const anatomy = RACE_ANATOMY[raceId] ?? RACE_ANATOMY.human;
  const profile = Math.abs(direction.x) > 0.9;
  const perpendicular = profile
...
function drawBodyV5(image, p, body, raceId = 'human') {
  const profile = Math.abs(p.direction.x) > 0.9;
  const back = p.direction.y < -0.45;
  const diagonal = Math.abs(p.direction.x) > 0.45 && !profile;
  const side = p.direction.x < 0 ? -1 : 1;
  ...
  if (profile) {
    ...
    const halfWidth = 6.35 + broad + feminine;
    const top = y - 7.25 - tall;
    polygon(image, [
      [x - 3.8, top], [x + 3.8, top],
      [x + halfWidth - 0.5, y - 5.1], [x + halfWidth, y - 1.4],
      [x + halfWidth - 0.25, y + 2.7], [x + 3.7, y + 5.35],
      [x, y + 6.4 + tall * 0.2], [x - 3.7, y + 5.35],
      [x - halfWidth + 0.25, y + 2.7], [x - halfWidth, y - 1.4],
...
function drawFaceV5(image, p, style, raceId = 'human') {
  ...
  if (profile) {
    ...
    const eyeY = y - 0.6;
    if (style === 'cheerful') {
      line(image, eyeX - side * 0.8, eyeY, eyeX + side * 0.8, eyeY + 0.15, C.outline, 0.8);
      put(image, x + side * 2.25, y + 1.25, '#e99a8f');
      put(image, x + side * 3.05, y + 1.55, C.skinShade);
    } else {
      rect(image, eyeX - 0.75, eyeY - 0.8, 1.5, 2.2, C.outline);
      rect(image, eyeX - 0.15 + side * 0.25, eyeY - 0.35, 0.8, 1.25, eyeColor);
      detail(image, eyeX + side * 0.25, eyeY - 0.3, C.skinLight);
      line(image, eyeX - side, eyeY - 1.65, eyeX + side * 0.9, eyeY - (style === 'focused' ? 1.9 : 1.6), C.outlineSoft, style === 'focused' ? 0.9 : 0.65);
    }
    ...
    for (const eye of eyes) {
      ...
      if (style === 'cheerful') {
        line(image, eye.x - width * 0.6, eyeY, eye.x + width * 0.6, eyeY + 0.15, C.outline, 0.8);
      } else {
        rect(image, eye.x - width / 2, eyeY - 0.8, width, 2.2, C.outline);
        rect(image, eye.x - 0.1 + (diagonal ? side * 0.18 : 0), eyeY - 0.35, 0.75, 1.25, eyeColor);
        detail(image, eye.x + (diagonal ? side * 0.2 : -0.15), eyeY - 0.3, C.skinLight);
    ...
    if (style === 'cheerful') {
      ...
      put(image, centerX - 4.0, eyeY + 2.6, '#e99a8f');
      put(image, centerX + 4.0, eyeY + 2.6, '#e99a8f');
    } else {
...
function drawHairV5(image, p, body, style, raceId = 'human') {
  const profile = Math.abs(p.direction.x) > 0.9;
  const back = p.direction.y < -0.45;
  const diagonal = Math.abs(p.direction.x) > 0.45 && !profile;
  const side = p.direction.x < 0 ? -1 : 1;
...
function drawLayer(kind, options, row, column) {
  const image = frame();
  const p = poseV5(row, column, options.raceId);
  if (kind === 'body') drawBodyV5(image, p, options.body, options.raceId);
  if (kind === 'face') drawFaceV5(image, p, options.style, options.raceId);
  if (kind === 'heritage') drawHeritage(image, p, options.style, options.raceId);
  if (kind === 'hair') drawHairV5(image, p, options.body, options.style, options.raceId);
...
for (const raceId of races) {
  ...
  const manifest = {
    ...
    race: raceId,
    allowedClasses,
    provenance: 'Authored from scratch by scripts/generate-human-fresh-assets.mjs using the v5 shared directional rig; reads no prior character sprite files.',
    frame: {
      size: frameSize,
      logicalSize,
      pixelScale: scale,
    ...
    architecture: {
      ...
      anchorContract: 'Every race and layer is drawn independently from one v5 eight-direction screen-space skeleton with shared head, hand, hip, shoulder, and foot anchors.',
      faceContract: 'Front, diagonal, profile, rear-diagonal, and back skull volumes are authored separately; faces use sparse two-pixel marks and never render through rear views.',
  ...
  writeFileSync(join(outputRoot, 'README.md'), [
    ...
    '- New v5 48x48 logical skeleton rendered as crisp two-pixel clusters into 96x96 frames.',
    ...
    '- Chunky high-contrast construction matches the readable classic mage style target without copying an old sprite.',
    '- Cardinal profiles use a restrained forehead/nose/chin silhouette; diagonal and back views share the same compact skull volume.',
    '- Sparse eyes, nose and mouth pixels avoid the oversized mask-like faces of the archived v4 model.',
