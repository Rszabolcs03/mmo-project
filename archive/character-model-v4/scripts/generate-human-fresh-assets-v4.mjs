...
const modelId = 'adventurer-fresh-v4';
const modelVersion = 4;

// v4 keeps the runtime-compatible 96px frame, but authors directly on that
// canvas. Geometry remains expressed in the established 48-unit design space,
// so existing animation/equipment anchors do not move. The extra native pixels
// are available to faces, silhouettes and hair instead of being a forced 2x
// enlargement of a much coarser image.
const designSize = 48;
const detailScale = 2;
...
for (const raceId of races) {
  ...
  const manifest = {
    ...
    architecture: {
      ...
      anchorContract: 'Every race and layer is drawn independently from the same corrected screen-space skeleton pose, including authored profile depth and native-resolution facial landmarks.',
