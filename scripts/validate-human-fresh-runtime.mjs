...
if (
  gameData.CHARACTER_SPRITE_VERSION !== 'adventurer-fresh-v6'
...
) throw new Error('Runtime animation constants do not match adventurer-fresh-v6.');
...
for (const [raceId, race] of Object.entries(gameData.RACES)) {
  ...
  if (manifest.model !== 'adventurer-fresh-v6' || manifest.version !== 6 || manifest.race !== raceId) {
    throw new Error(`${raceId} manifest is not adventurer-fresh-v6.`);
  }
    ...
    for (const body of bodies) {
      for (let option = 0; option < 4; option += 1) {
        ...
        const expected = {
          ...
          headwear: `${raceId}-fresh-${classId}-${body}-${outfits[option]}`,
        ...
        if (appearance.spriteModel !== 'adventurer-fresh-v6'
          || appearance.characterSpriteModel !== 'adventurer-fresh-v6') {
...
if (
  ...
  || migrated.spriteModel !== 'adventurer-fresh-v6'
  || migrated.characterSpriteModel !== 'adventurer-fresh-v6'
) throw new Error('Old non-human saves are not migrated onto fresh assets.');

console.log('Validated adventurer-fresh-v6 runtime: all five races use race-specific fresh layers and independent class headwear for allowed classes only, every layer resolves inside its race root, creation/Damage uses one sword, Tank adds the matching shield, and old saves migrate forward.');
