...
const modelId = 'adventurer-fresh-v3';
...
function drawBody(image, p, body, raceId = 'human') {
  ...
  if (profile) {
    const x = p.head.x;
    const y = p.head.y;
    const neckWidth = raceId === 'orc' || raceId === 'dwarf' ? 3.2 : raceId === 'undead' ? 2.2 : 2.7;
    // The profile is an authored side silhouette rather than a squeezed front
    // head. A short neck, rear skull plane, brow, nose and jaw give equipment
    // and hair stable landmarks without changing the shared frame anchor.
    polygon(image, [
      [x - side * neckWidth, y + 5], [x + side * (neckWidth - 0.5), y + 5],
      [x + side * (neckWidth - 1), y + 11], [x - side * (neckWidth + 0.5), y + 11],
    ], C.outline);
    polygon(image, [
      [x - side * (neckWidth - 1), y + 5], [x + side * (neckWidth - 1.2), y + 5],
      [x + side * (neckWidth - 1.5), y + 10], [x - side * (neckWidth - 0.2), y + 10],
    ], C.skinShade);
    ellipse(image, x - side * 0.25, y, radiusX + 0.35, radiusY, C.outline);
    ellipse(image, x - side * 0.15, y + 0.2, radiusX - 0.65, radiusY - 1.05, C.skinShade);
    ellipse(image, x + side * 0.35, y - 0.55, radiusX - 1.35, radiusY - 2.05, C.skin);
    const noseReach = raceId === 'elf' ? 1.35 : raceId === 'dwarf' ? 0.8 : raceId === 'orc' ? 1.1 : raceId === 'undead' ? 0.65 : 1;
    polygon(image, [
      [x + side * (radiusX - 2.2), y - 2.3],
      [x + side * (radiusX + noseReach), y - 0.2],
      [x + side * (radiusX + noseReach - 0.2), y + 1.2],
      [x + side * (radiusX - 1.8), y + 2.1],
    ], C.outline);
    polygon(image, [
      [x + side * (radiusX - 2), y - 1.4],
      [x + side * (radiusX + noseReach - 0.15), y],
      [x + side * (radiusX - 1.5), y + 1.2],
    ], C.skin);
    line(image, x + side * (radiusX - 2.1), y + 3, x + side * (radiusX - 3.1), y + 6, C.skinDeep, 1);
    ellipse(image, x - side * (radiusX - 0.7), y + 1, 1.25, 1.8, C.outline);
    put(image, x - side * (radiusX - 0.7), y + 1, C.skinShade);
    line(image, x - side * 3, y - 6, x + side * 1.5, y - 6, C.skinLight, 1);
    line(image, x - side * (radiusX - 2), y - 3, x - side * (radiusX - 2), y + 4, C.skinDeep, 1);
    line(image, x + side, y + radiusY - 2.4, x - side * 2.5, y + radiusY - 1.25, C.skinDeep, 1);
    put(image, x + side * (radiusX - 0.5), y + 0.5, C.skinDeep);
  } else {
...
function drawFace(image, p, style, raceId = 'human') {
  ...
  if (profile) {
    // Cardinal sides use one readable eye plus a brow/nose/mouth/chin chain
    // aligned to the authored profile silhouette in the body layer.
    const eyeX = p.head.x + side * (raceId === 'orc' ? 2.8 : raceId === 'elf' ? 2.5 : 2.2);
    ...
    put(image, p.head.x + side * 5, p.head.y + 1, C.outlineSoft);
    ...
    line(
      ...
      p.head.y + 5,
      p.head.x + side * 3,
      p.head.y + (style === 'cheerful' ? 4 : 5),
      C.outlineSoft,
      1,
    );
    put(image, p.head.x + side * 2, p.head.y + 6, style === 'focused' ? C.outline : C.skinLight);
    put(image, p.head.x - side * 4, p.head.y + 1, C.skinDeep);
    if (style === 'scarred') {
    ...
    if (raceId === 'elf') {
      ...
      put(image, p.head.x + side * 4, p.head.y + 3, C.skinLight);
    }
    if (raceId === 'dwarf') {
      line(image, eyeX - side * 2, eyeY - 3, eyeX + side, eyeY - 3, C.outline, 2);
      put(image, p.head.x + side * 3, p.head.y + 2, C.skinDeep);
      line(image, p.head.x - side * 2, p.head.y + 4, p.head.x + side * 2, p.head.y + 6, C.skinDeep, 1);
    }
    if (raceId === 'orc') {
      ...
      put(image, p.head.x + side * 5, p.head.y + 4, C.skinLight);
    }
    if (raceId === 'undead') {
      ...
      put(image, p.head.x - side * 2, p.head.y - 4, C.skinLight);
    }
    return;
  }
  ...
  put(image, centerX - 1, eyeY + 1, C.skinLight);
  put(image, centerX + 1, eyeY + 3, C.skinShade);
  if (style === 'cheerful') {
  ...
  put(image, centerX, eyeY + 6, C.skinLight);
  put(image, centerX - 5, eyeY + 2, C.skinShade);
  put(image, centerX + 5, eyeY + 2, C.skinDeep);
  if (style === 'scarred') {
...
function drawHeritage(image, p, style, raceId = 'human') {
  ...
  if (raceId === 'human') {
    if (style === 'road-freckles') {
      const cheekX = x + visibleSide * (profile ? 3 : 4);
      for (const [dx, dy] of [[-1, 2], [1, 2], [0, 3], [2, 3]]) put(image, cheekX + dx, y + dy, '#b86e58');
    } else if (style === 'temple-scar') {
      const templeX = x + visibleSide * (profile ? 1 : 4);
      line(image, templeX - visibleSide, y - 4, templeX + visibleSide, y + 2, '#8f4d4d', 1);
      put(image, templeX, y - 1, C.skinLight);
    } else if (style === 'guild-mark') {
      const markX = x + visibleSide * (profile ? 2 : 4);
      polygon(image, [[markX, y + 1], [markX + 1, y + 2], [markX, y + 3], [markX - 1, y + 2]], C.goldDeep);
      put(image, markX, y + 2, C.goldLight);
    }
    return;
  }
  ...
  if (raceId === 'elf') {
    if (style === 'moon-sigil') {
      const markX = x + visibleSide * (profile ? 1 : 4);
      line(image, markX - visibleSide, y + 1, markX, y + 3, '#7dd3fc', 1);
      put(image, markX + visibleSide, y + 2, C.goldLight);
    } else if (style === 'leaf-filigree') {
      const markX = x + visibleSide * (profile ? 2 : 4);
      line(image, markX, y, markX - visibleSide, y + 5, '#5f9f75', 1);
      put(image, markX + visibleSide, y + 2, '#b8e3cf');
      put(image, markX - visibleSide * 2, y + 4, '#b8e3cf');
    } else if (style === 'star-circlet') {
      line(image, x - (profile ? side * 3 : 5), y - 4, x + (profile ? side * 3 : 5), y - 4, C.goldDeep, 1);
      put(image, x + (profile ? side : 0), y - 4, C.goldLight);
      put(image, x + (profile ? side : 0), y - 5, '#c4b5fd');
    }
    return;
  }
  ...
  if (raceId === 'dwarf') {
    if (style === 'forge-smudge') {
      const cheekX = x + visibleSide * (profile ? 3 : 4);
      line(image, cheekX - visibleSide, y + 1, cheekX + visibleSide, y + 3, '#6b351f', 2);
      put(image, cheekX, y + 2, '#9a4f2f');
    } else if (style === 'clan-rune') {
      const runeX = x + visibleSide * (profile ? 2 : 4);
      line(image, runeX, y, runeX, y + 5, '#d6b25e', 1);
      line(image, runeX - visibleSide, y + 1, runeX + visibleSide, y + 2, C.goldLight, 1);
      put(image, runeX - visibleSide, y + 4, C.goldDeep);
    } else if (style === 'brass-studs') {
      const studX = x + visibleSide * (profile ? 4 : 5);
      for (const dy of [-2, 1, 4]) {
        put(image, studX, y + dy, C.goldDeep);
        put(image, studX - visibleSide, y + dy, C.goldLight);
      }
    }
    return;
  }
  ...
  if (raceId === 'orc') {
    if (style === 'war-paint') {
      const paintX = x + visibleSide * (profile ? 2 : 4);
      line(image, paintX - visibleSide, y - 3, paintX + visibleSide, y + 4, '#a52f2a', 2);
      put(image, paintX, y, '#e05044');
    } else if (style === 'fang-stripe') {
      const paintX = x + visibleSide * (profile ? 3 : 4);
      line(image, paintX - visibleSide, y - 2, paintX + visibleSide, y + 3, '#f1e5c8', 1);
      line(image, paintX, y + 1, paintX - visibleSide, y + 5, '#9f7f54', 1);
    } else if (style === 'bone-studs') {
      const studX = x + visibleSide * (profile ? 4 : 5);
      for (const dy of [-2, 1, 4]) put(image, studX - visibleSide * (dy % 2), y + dy, '#f1e5c8');
    }
    return;
  }
  ...
  if (raceId === 'undead') {
    if (style === 'grave-cracks') {
      const crackX = x + visibleSide * (profile ? 2 : 3);
      line(image, crackX, y - 2, crackX - visibleSide * 2, y + 2, '#6b746c', 1);
      line(image, crackX - visibleSide, y + 1, crackX + visibleSide, y + 5, '#6b746c', 1);
    } else if (style === 'ritual-stitches') {
      const stitchX = x + visibleSide * (profile ? 2 : 3);
      line(image, stitchX - visibleSide * 2, y + 2, stitchX + visibleSide * 2, y + 4, C.outlineSoft, 1);
      for (const offset of [-1, 1]) put(image, stitchX + visibleSide * offset, y + 3, '#d8ddd0');
    } else if (style === 'spectral-veins') {
      const veinX = x + visibleSide * (profile ? 2 : 3);
      line(image, veinX, y - 3, veinX - visibleSide, y + 4, '#67e8f9', 1);
      put(image, veinX + visibleSide, y, '#a7f3d0');
      put(image, veinX - visibleSide * 2, y + 3, '#a7f3d0');
    }
  }
}
...
function drawLayer(kind, options, row, column) {
  ...
  if (kind === 'beard') drawBeard(image, p, options.style, options.raceId);
...
for (const raceId of races) {
  ...
  const manifest = {
    ...
    architecture: {
      layers: ['cape', 'body', 'outfit', 'face', 'heritage', 'beard', 'hair', 'offhand', 'weapon'],
      anchorContract: 'Every race and layer is drawn independently from the same corrected screen-space skeleton pose, including authored profile depth.',
