...
if (manifest.model !== 'adventurer-fresh-v3' || manifest.version !== 3) {
  throw new Error(`Expected adventurer-fresh-v3, got ${manifest.model} v${manifest.version}.`);
}
...
if (manifest.architecture.layers.join(',') !== 'cape,body,outfit,face,heritage,beard,hair,offhand,weapon') {
...
if (paths.size !== 153) throw new Error(`Expected 153 unique fresh atlases, got ${paths.size}.`);
...
for (let option = 0; option < heritageImages.length; option += 1) {
  if (alphaCount(heritageImages[option], 0, 0) < 8) throw new Error(`Heritage option ${option} is not visible from the front.`);
...
console.log('Validated 153 from-scratch human adventurer-fresh-v3 atlases: authored side silhouettes and torso depth, detailed directional faces, race-owned heritage layers, anchored equipment, four-phase walks and attacks, eight directions, one-sword DPS weapons, and independent tank offhands.');
