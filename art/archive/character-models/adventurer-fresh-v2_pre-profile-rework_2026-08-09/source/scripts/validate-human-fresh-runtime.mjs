...
if (
  gameData.CHARACTER_SPRITE_VERSION !== 'adventurer-fresh-v2'
...
) throw new Error('Runtime animation constants do not match adventurer-fresh-v2.');
...
for (const [raceId, race] of Object.entries(gameData.RACES)) {
  ...
  if (manifest.model !== 'adventurer-fresh-v2' || manifest.version !== 2 || manifest.race !== raceId) {
    throw new Error(`${raceId} manifest is not adventurer-fresh-v2.`);
  }
  ...
  for (const classId of race.allowedClasses) {
    for (const body of bodies) {
      for (let option = 0; option < 4; option += 1) {
        ...
        if (appearance.spriteModel !== 'adventurer-fresh-v2'
          || appearance.characterSpriteModel !== 'adventurer-fresh-v2') {
...
if (
  ...
  || migrated.spriteModel !== 'adventurer-fresh-v2'
  || migrated.characterSpriteModel !== 'adventurer-fresh-v2'
) throw new Error('Old non-human saves are not migrated onto fresh assets.');

console.log('Validated adventurer-fresh-v2 runtime: all five races use race-specific fresh layers for allowed classes only, all cosmetics resolve inside their race root, creation/Damage uses one sword, Tank adds the matching shield, and old saves migrate forward.');
