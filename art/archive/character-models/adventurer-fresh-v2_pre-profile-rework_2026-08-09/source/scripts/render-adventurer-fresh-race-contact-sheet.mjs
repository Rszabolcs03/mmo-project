...
for (const [raceId, classId] of examples) {
  ...
  for (const body of bodies) {
    ...
    for (let row = 0; row < directions; row += 1) {
      ...
      const paths = back
        ? [layers.weapon, layers.body, layers.outfit, layers.face, layers.hair]
        : classId === 'hunter'
          ? [layers.body, layers.outfit, layers.weapon, layers.face, layers.hair]
          : [layers.body, layers.outfit, layers.face, layers.hair, layers.weapon];
